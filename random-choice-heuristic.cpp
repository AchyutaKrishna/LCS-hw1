...
tuple<int, int> identifyNextProp(Form f){ // Heuristic function - random choice heuristic
    int prop;
    srand(222);
    int nv = 1000000;
    while(nv != 0){
        nv--;
        int ptmp = rand()%nprop+1;
        if(f.propsCount.find(ptmp) != f.propsCount.end()){
            if(f.propsCount[ptmp] >= 0){
                prop = ptmp;
                break;
            }
        }
...
int main(){
    ...
    int c = 100;
    int nvs = 125, lvs = t[i];
    string f1 = "formulas/f" + to_string(nvs) + "_" + to_string(lvs) + "/resrand.txt";
    ofstream MyFile(f1);
    vector<int> countv;
